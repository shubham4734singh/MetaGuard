...
import { historyApi, FileHistory } from "../services/api";
...
interface FileRecord {
  ...
  date: string;
  time: string;
  status: "Cleaned" | "Failed" | "Pending" | "Analyzed";
}
...
export function HistoryPage({
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        const transformedData: FileRecord[] = data.map((item: FileHistory) => ({
          id: item.id.toString(),
          name: item.file_name,
          type: item.file_type,
          size: item.file_size_display,
          risk: (item.risk_level || "Low") as RiskLevel,
          metadataCount: item.metadata_count,
          date: item.date,
          time: item.time,
          status: item.status,
        }));
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "Analyzed": return "text-blue-400 bg-blue-500/10 border-blue-500/30";
  ...
  // Filter data
  const filteredData = historyData.filter((file) => {
  ...
  const fileTypes = ["All", ...Array.from(new Set(historyData.map((f) => f.type)))];
  ...
  const highRiskFiles = historyData.filter((f) => f.risk === "High").length;
  const totalMetadataRemoved = historyData.reduce((sum, f) => sum + f.metadataCount, 0);
            ...
            <Card className="bg-gradient-to-br from-zinc-900/50 to-zinc-950/50 border-zinc-800 overflow-hidden">
              ...
              {loading ? (
                <div className="p-12 text-center">
                  ...
                  <p className="mt-4 text-zinc-400">Loading history...</p>
                </div>
              ) : error ? (
                <div className="p-12 text-center">
                ...
                <div className="p-12 text-center">
                  ...
                  <p className="text-zinc-500 text-sm">Analyze or clean a file to see it here</p>
                </div>
              ) : (
              ...
              {!loading && !error && historyData.length > 0 && filteredData.length === 0 && (
